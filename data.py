...
class Dataset(data.Dataset):
    def __init__(self, img_list_file, crop_size=(256, 256)):
        ...
        for line in lines:
            ...
            self.img_list.append(os.path.join(data_offset,img_name))
            self.gt_list.append(os.path.join(data_offset,gt_name))
    ...
    def __getitem__(self, idx):
        clear_img = torch.from_numpy(np.array(cv2.imread(self.gt_list[idx])).transpose((2, 0, 1)))
        blurry_img = torch.from_numpy(np.array(cv2.imread(self.img_list[idx])).transpose((2, 0, 1)))
